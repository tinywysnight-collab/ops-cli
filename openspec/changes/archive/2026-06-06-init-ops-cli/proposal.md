...
- `opsx init zsh` / `opsx init bash` / `opsx init cmd` + `opsx shell-switch …`: one-time shell function or wrapper plus shell-consumed assignment lines, so switches take effect in the parent shell (manual fallback supported).
...
Out of scope for v1 (deferred to v2): background token refresh/daemon (N1), AWS Org/EKS auto-discovery (N2), per-account default region (N3), shell completion (N4), interactive selector (N5), `opsx exec` (N6), fish generator (N7).
...
- `shell-integration`: `opsx init zsh` / `opsx init bash` / `opsx init cmd` function/wrapper generators and `opsx shell-switch …` assignment emitters (strict stdout = environment assignments only, everything else to stderr) that deliver `AWS_PROFILE`/`KUBECONFIG`/`OPSX_MODE` to the parent shell. [FR7, FR8]
...
- **Shell environment**: appends a one-time `opsx()` function to the user's zsh or Bash rc, or installs an `opsx.cmd` wrapper for Command Prompt; exports/sets `AWS_PROFILE`/`KUBECONFIG`/`OPSX_MODE` per terminal.
