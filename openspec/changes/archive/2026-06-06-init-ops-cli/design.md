...
- AWS Org / EKS auto-discovery (N2), per-account default region (N3), shell completion (N4), interactive selector (N5), `opsx exec` (N6), fish generator (N7).
...
**D7 — Shell integration via `shell-switch` + guarded assignment application (M8).** One-time POSIX shell functions installed by `opsx init zsh` or `opsx init bash` route switching subcommands (`use`, `kube`, `mode`) through `shell-switch`, capture its stdout, verify the command succeeded, then `eval` only the export lines. The Bash function covers Git Bash on Windows. Command Prompt uses `opsx init cmd` to generate an `opsx.cmd` PATH-priority wrapper, which routes switchers through `shell-switch --shell cmd` and applies native `set "KEY=value"` lines in the current `cmd.exe`. Non-switching subcommands run the bare binary. Wrappers must also recognize switching subcommands when global flags appear before them (`opsx --mode opr use dev`). `shell-switch` prints *only* environment assignment lines to stdout; everything else (prompts, MFA number, logs, errors) goes to stderr. Manual `eval "$(opsx shell-switch use dev)"` fallback always works for POSIX portability, with shell-specific fallbacks for PowerShell/cmd.
...
- **`shell-switch` stdout pollution or swallowed shell errors would break parent-shell updates** → Mitigation: strict discipline — stdout carries only environment assignment lines; a single emitter (`shell/emit.go`) is the sole stdout-for-shell path; everything else to stderr; wrappers capture command output and return non-zero on shell-switch failure before applying assignments.
