...
- [x] 18.3 **(entra-auth/live) Company-machine live verification.** On the proxy-gated company machine, run `opsx login` against the real Entra/ADFS/MFA endpoints and compare the request/response assumptions against the legacy Python script. Keep the env/file assertion escape hatch available for recovery and off-network testing. _(Done: verified on the company machine through the correct flow; the env/file assertion escape hatch remains available.)_
...
## 20. opsx kube default kubeconfig merge — 2026-06-06

The kubeconfig parallel of §19. `opsx use` already overwrites `[default]` so `aws` works with no
`AWS_PROFILE`; `opsx kube` still needs `KUBECONFIG` to point `kubectl` at the per-cluster file,
which only the shell function can inject. So `opsx kube` also merges the cluster into the default
`~/.kube/config` and sets it as current-context, letting `kubectl` target the cluster with no
`KUBECONFIG`, no shell function, no `eval` — covering locked-down PowerShell / Command Prompt.
Single-user tool: the merge is unconditional, via `aws eks update-kubeconfig` against
`~/.kube/config` (so its own merge preserves unrelated entries). See cluster-switching "Default
kubeconfig merge". Re-apply TDD red-green-refactor: failing test first.

- [x] 20.1 **(paths) Default kubeconfig path helper.** Write a failing test first, then add a `paths` helper that resolves `~/.kube/config` (honoring `$HOME` / test overrides), so the kube flow has a single source of truth for the default kubeconfig location.
- [x] 20.2 **(cluster-switching) Merge the cluster into `~/.kube/config`.** Write failing tests first (fake kube service), then have `switchCluster` also invoke `aws eks update-kubeconfig` targeting `~/.kube/config` with `--profile <alias.mode>` and a friendly `--alias <cluster>` so the AWS CLI merges/creates the entry and sets `current-context`. This runs in addition to the existing per-(cluster,mode) file write. Tests assert: a second update-kubeconfig targets the default path with the cluster alias and profile; the per-cluster file write is unchanged.
- [x] 20.3 **(cluster-switching) Merge is additive and non-destructive.** Write failing tests first, then ensure the default-merge step does not pass any destructive flag and relies on the AWS CLI merge to preserve unrelated `clusters`/`contexts`/`users`; if `~/.kube` does not exist, the flow must not fail (create the dir or rely on the CLI). Tests assert unrelated entries survive and a missing `~/.kube` is handled.
- [x] 20.4 **(docs) Document the default kubeconfig merge.** Update README (and PRD/architecture): `opsx kube` now merges into `~/.kube/config` and sets current-context so `kubectl` works with no `KUBECONFIG` (locked-down PowerShell / Command Prompt); `~/.kube/config` reflects the latest `opsx kube` (no per-terminal isolation on that path; function shells stay isolated via `KUBECONFIG`); `opsx logout` does not edit `~/.kube/config`.
- [x] 20.5 **(verification) Gate the change.** Run `gofmt`, `golangci-lint run ./...`, `go vet ./...`, `go test -race -cover ./...`, and `make build`; confirm the existing per-cluster `KUBECONFIG`/isolation path is unchanged and the new default-merge path is covered.
