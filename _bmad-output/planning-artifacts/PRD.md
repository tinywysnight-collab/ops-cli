...
| M5 | **`opsx kube <cluster-alias>`** | Auto-ensure the cluster's account creds are ready (current mode); run `aws eks update-kubeconfig` for the alias's region + real name into `~/.config/opsx/kube/<mode>/<encoded-cluster>.yaml` with `--profile <alias.mode>`; export both `AWS_PROFILE` and per-terminal `KUBECONFIG`. `helm` follows the same context automatically. Also merges the cluster into the default `~/.kube/config` (current-context set) so `kubectl` works with no `KUBECONFIG` in shells where opsx cannot inject env vars — the kubeconfig parallel of M13; see the default-profile note. |
